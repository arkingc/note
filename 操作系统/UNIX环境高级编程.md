...
* **lstat**：返回符号链接`pathname`（而不是由该符号链接引用的文件）的有关信息，存在`buf`中
...
一个给定的FIFO有多个写进程是场景的。这意味着，如果不希望多个进程所写的数据交叉，则必须考虑原子写操作。和管道一样，常量`PIPE_BUF`说明了可被原子地写到FIFO的最大数据量
